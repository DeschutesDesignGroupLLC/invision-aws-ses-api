...
class _logs extends \IPS\Dispatcher\Controller
{
	...
	{		
		// Create the table
		$table = new \IPS\Helpers\Table\Db('awsses_bounce_logs', \IPS\Http\Url::internal('app=awsses&module=bounces&controller=logs'));
		...
		$table->rowButtons = function ($row) {
			return array(
				'delete'    => array(
					'title' => 'delete',
					'icon'  => 'times-circle',
					'link'  => \IPS\Http\Url::internal('app=awsses&module=bounces&controller=logs&do=delete')->setQueryString('id', $row['id']),
					'data'  => array( 'delete' => '' )
				)
			);
		};
	...
	protected function delete()
	{
		...
		try {
			// Load the log
			$log = \IPS\awsses\Bounce\Log::load(\IPS\Request::i()->id);
		}
		...
		// Redirect
		\IPS\Output::i()->redirect(\IPS\Http\Url::internal('app=awsses&module=bounces&controller=logs'), 'deleted');
	}
}